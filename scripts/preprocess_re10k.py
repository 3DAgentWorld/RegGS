#!/usr/bin/env python3
"""Convert RealEstate10K scene metadata to RegGS sample_data format.

Input txt format (RE10K):
...
class CameraRecord:
    ...
    k1: float
    k2: float
    rotation_w2c: np.ndarray  # shape (3, 3)
    translation_w2c: np.ndarray  # shape (3,)
    ...
    for idx, line in enumerate(lines[1:], start=2):
        ...
        records.append(
            CameraRecord(
                ...
                translation_w2c=translation_w2c,
            )
        )
...
def _fallback_rotmat_to_xyzw(rotation_c2w: np.ndarray) -> list[float]:
    """Convert rotation matrix to quaternion [x, y, z, w] without roma."""
    m = rotation_c2w
...
def rotation_w2c_to_quaternion_xyzw(rotation_w2c: np.ndarray) -> list[float]:
    """Convert world-to-camera rotation to camera-to-world quaternion [x,y,z,w]."""
...
def build_cameras_json(
    ...
    for i, (rec, image_name) in enumerate(zip(records, image_names)):
        # World-to-camera [R|t] -> camera center in world: C = -R^T t.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Convert RealEstate10K scene to RegGS sample_data format"
    )
    parser.add_argument(
        "--scene-txt",
        type=Path,
        required=True,
        help="Path to a RealEstate10K scene txt file (e.g., datasets/RealEstate10K/test/<scene>.txt)",
    )
    parser.add_argument(
        "--out-root",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Original frame width before 256 preprocessing; used to scale fx",
    )
    parser.add_argument(
        "--target-width",
        type=int,
